...
const API_URL = 'http://www.mocky.io/v2/5a6224f2310000122cde7f24';
...
describe('Actions: Agast', () => {
    ...
    it('creates SAVE_AGAST_SUCCESS when save Agast has been done', () => {
        ...
        const expectedActions = [
            ...
            { type: types.SAVE_AGAST_SUCCESS, body }
        ];
        const store = mockStore({});

        return store.dispatch(actions.saveAgast(body)).then(() => {
            // return of async actions
            expect(fetchMock.called(API_URL)).toBe(true);
            expect(store.getActions()).toEqual(expectedActions);
        });
    ...
    it('creates SAVE_AGAST_FAILURE when fetching Agast has been done', () => {
        const body = {code: '001', description: 'New Agast'};

        fetchMock
            .postOnce(API_URL, 500);

        const expectedActions = [
            { type: types.SAVE_AGAST_REQUEST },
            { type: types.SAVE_AGAST_FAILURE, ex: 'Error: {}' }
        ];
        const store = mockStore({});
