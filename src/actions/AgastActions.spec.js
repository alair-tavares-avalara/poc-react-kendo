...
process.env.REACT_APP_AGAST_MOCK_API_URL = 'http://www.mocky.io/v2';
...
describe('Actions: Agast', () => {
    beforeAll(() => fetchMock.config.Request = Request);
    ...
    it('creates FETCH_AGAST_SUCCESS when fetching Agast has been done', () => {
        const API_URL = `${process.env.REACT_APP_AGAST_MOCK_API_URL}/5a6224f2310000122cde7f24?`;

        fetchMock
            .getOnce(API_URL, { body: mockBody });
        ...
        return store.dispatch(actions.fetchAgast()).then(() => {
            ...
            expect(fetchMock.called(API_URL)).toBe(true);
    ...
    it('creates SAVE_AGAST_SUCCESS when save Agast has been done', () => {
        const API_URL = `${process.env.REACT_APP_AGAST_MOCK_API_URL}/5a6224f2310000122cde7f24`;
        const body = { code: '001', description: 'New Agast' };
        const expectResponse = {status: 200, data: body};
        fetchMock
            .postOnce(API_URL, body);

        const expectedActions = [
            { type: types.SAVE_AGAST_REQUEST },
            { type: types.SAVE_AGAST_SUCCESS, body: expectResponse }
        ];
        const store = mockStore({});

        return store.dispatch(actions.saveAgast(body)).then((response) => {
            // return of async actions
            expect(response.body.status).toEqual(expectResponse.status);
            response.body.data.then(d => expect(d).toEqual(expectResponse.data));
            expect(fetchMock.called(API_URL)).toBe(true);

            const actionsResponse = store.getActions();
            expect(actionsResponse[0]).toEqual(expectedActions[0]);
            expect(actionsResponse[1].type).toEqual(expectedActions[1].type);
            expect(actionsResponse[1].body.status).toEqual(expectResponse.status);
            actionsResponse[1].body.data.then(d => expect(d).toEqual(expectedActions[1].body.data));
        });
    });
});